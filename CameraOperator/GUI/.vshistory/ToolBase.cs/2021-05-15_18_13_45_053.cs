...
namespace CameraOperatorMod.GUI
{
    ...
    abstract class ToolBase : MonoBehaviour, ITool
    {
        ...
        public virtual void Start()
		{
			...
			tabStrip.tabPages = Window.AddUIComponent<UITabContainer>();
			...
			foreach (var v in Tabs.Select((tab, index) => new { tab, index }))
			{
				{
					var btn = tabStrip.AddTab(v.tab.name, true);
					tabStrip.selectedIndex = v.index; // important for setting current target obj

					btn.tabIndex = v.index; // misc value
				...
				var page = container.components[v.index] as UIPanel;

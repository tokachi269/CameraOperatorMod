...
namespace CameraOperatorMod.GUI
{
    public static class Helper
    {
        ...
        public static UICheckBox AddButton(ref UIPanel parent, string label, string tooltip, bool initialValue, PropertyChangedEventHandler<bool> onCheckChanged)
        {
            var button = parent.AddUIComponent<UICheckBox>();
            button.anchor = UIAnchorStyle.Top | UIAnchorStyle.Left | UIAnchorStyle.Right;

            button.label = button.AddUIComponent<UILabel>();
            button.label.anchor = UIAnchorStyle.Top | UIAnchorStyle.Left | UIAnchorStyle.Right;
            ...
            button.label.padding.left = button.label.font.size + 6;

            button.label.relativePosition = new Vector2(0, -(button.label.font.size - 10));
            button.label.FitTo(parent);
            button.height = button.label.height;

            var uncheckedSprite = button.AddUIComponent<UISprite>() as UISprite;
            uncheckedSprite.spriteName = "AchievementCheckedFalse";
            //uncheckedSprite.relativePosition = new Vector2(indentPadding.HasValue ? indentPadding.Value : 0, 2.5f);
            uncheckedSprite.anchor = UIAnchorStyle.Top | UIAnchorStyle.Left;
            uncheckedSprite.width = uncheckedSprite.height = button.label.font.size;
            ...
            button.checkedBoxObject = checkedSprite;

            button.eventCheckChanged += onCheckChanged;
            button.isChecked = initialValue;
            // box.isChecked = initialValue;
            //if (initialValue) box.SimulateClick();
            return button;
        }
